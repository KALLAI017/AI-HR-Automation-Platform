...
**Access**: Admin Portal → 📈 Interview Results**Location**: `interview_results/{candidate_id}/interview_{timestamp}.json`✅ **Organized**: Separate folders per candidate  ✅ **Exportable**: Single JSON file for all data  ✅ **Complete**: Includes scores, conversation, analysis  ✅ **Accessible**: View via admin UI or Python API  ✅ **Permanent**: JSON files persist across app restarts  ✅ **Automatic**: Results saved after every interview completion  ## Summary---3. **File Size**: Large exports may take time to generate2. **Check Path**: Look in `interview_results/all_interviews.json`1. **Admin Access**: Only admins can export### Export Not Working?3. **Check Timestamps**: Files named with creation date/time2. **Verify Candidate ID**: Use exact ID from application1. **Check Directory**: Look in `interview_results/` folder### Can't Find Results?4. **Check Disk Space**: Ensure sufficient storage available3. **Verify Import**: Make sure `interview_storage.py` exists2. **View Console**: Look for error messages in terminal1. **Check Permissions**: Ensure write access to project directory### Results Not Saving?## Troubleshooting---```Display Save Confirmation    ↓Write JSON to Disk    ↓InterviewStorage.save_interview_result()    ↓Prepare Data Structure    ↓Calculate Scores    ↓Interview Completion```### Data Flow   - Saves after interview completion   - `calculate_final_score()` function2. **technical_interview_ui.py** (Line ~540)   - Saves after code submission and analysis   - `submit_code_with_chat()` function1. **chat_interview_ui.py** (Line ~577)### Where Results Are Saved## Integration Points---3. **Cloud Storage**: Sync folder to cloud for redundancy2. **Version Control**: Consider backing up `interview_results/` folder1. **Regular Exports**: Use "Export All Results" in admin panel### Backup Recommendations- **Error Handling**: Graceful handling of read/write errors- **Pretty JSON**: Indented format (2 spaces) for readability- **UTF-8 Encoding**: Supports special characters- **Timestamp-Based Names**: Prevents overwriting previous interviews- **Directory Creation**: Directories created automatically on save### Automatic Features## File Safety---```    print(f"  {problem}: {avg:.1f}/100 (n={len(scores)})")    avg = sum(scores) / len(scores)for problem, scores in problem_scores.items():print("Problem Difficulty (by avg score):")# Calculate average score per problem        problem_scores[problem].append(score)            problem_scores[problem] = []        if problem not in problem_scores:                score = interview['scoring']['final_score']        problem = interview['interview_data']['problem']    for interview in interviews:    interviews = storage.load_candidate_interviews(candidate_id)for candidate_id in candidates:problem_scores = {}candidates = storage.get_all_candidates()storage = InterviewStorage()```python### Example 3: Analyze Problem Difficulty```    print(f"  {performer['id']}: Avg={performer['average']:.1f}, Best={performer['highest']:.1f}")for performer in top_performers:print("Top Performers:")top_performers.sort(key=lambda x: x['average'], reverse=True)# Sort by average score        })            'average': summary['average_score']            'highest': summary['highest_score'],            'id': candidate_id,        top_performers.append({    if summary['highest_score'] >= 80:    summary = storage.get_candidate_summary(candidate_id)for candidate_id in candidates:top_performers = []candidates = storage.get_all_candidates()storage = InterviewStorage()```python### Example 2: Find Top Performers```    print(f"{idx}. {problem}: {score:.1f}/100 on {date}")    date = interview['metadata']['date']    problem = interview['interview_data']['problem']    score = interview['scoring']['final_score']for idx, interview in enumerate(interviews, 1):print(f"Candidate has completed {len(interviews)} interviews")interviews = storage.load_candidate_interviews("CAND_001")storage = InterviewStorage()from interview_storage import InterviewStorage```python### Example 1: Check Candidate Progress## Data Access Examples---- Final calculated score- Code quality analysis- Test results- Interview Q&A responses**Saved data includes**:Results are **automatically saved** when you click **"✅ Finish Interview"**.### Quick Mode- Final calculated score- Code quality analysis- Test results- Hints used- Communication score- Approach quality score- Full conversation history**Saved data includes**:Results are **automatically saved** when you click **"✅ Submit"** in the code editor.### Chat Mode## Automatic Saving---- Date-sorted results- Data table with all interviews- Line chart showing score progression over time#### Performance Trends- Stages completed- Hints used- Code quality analysis- Test results (passed/total)- Score breakdown (tests, quality, interview)  - 🔴 Red: <60 (Fail)  - 🟡 Yellow: 60-79 (Pass)  - 🟢 Green: 80+ (Excellent)- Final score with color coding:- Problem name- Date and timestamp#### Individual Interview Details- **Problems Attempted**: Unique problems tried- **Highest Score**: Best performance- **Average Score**: Mean across all interviews- **Total Interviews**: Count of all attempts#### Summary View### Viewer Features   - Export all results to JSON   - View conversation history (chat mode)   - Expand individual interviews for detailed view   - See summary statistics (avg score, total interviews, etc.)   - Select specific candidate to view history   - View all candidates with saved interviews3. **Features**:2. **Navigate to**: Admin Portal → **📈 Interview Results**1. **Login as Admin**### Admin Portal - Interview Results Viewer## Accessing Results via UI---```print(f"All results exported to: {export_path}")export_path = storage.export_all_results("backup_20250129.json")```python### 6. Export All Results```print(f"Candidates with interviews: {', '.join(candidates)}")candidates = storage.get_all_candidates()```python### 5. List All Candidates```print(f"Problems attempted: {summary['problems_attempted']}")print(f"Highest score: {summary['highest_score']:.1f}")print(f"Average score: {summary['average_score']:.1f}")print(f"Total interviews: {summary['total_interviews']}")summary = storage.get_candidate_summary("CAND_001")```python### 4. Get Candidate Summary```    print(f"Latest score: {latest['scoring']['final_score']}")if latest:latest = storage.load_latest_interview("CAND_001")# Load only the latest interview    print(f"Problem: {interview['interview_data']['problem']}")    print(f"Score: {interview['scoring']['final_score']}")for interview in interviews:interviews = storage.load_candidate_interviews("CAND_001")# Load all interviews for a candidate```python### 3. Load Interview Results```print(f"Saved to: {filepath}"))    scoring_data=scoring_data    interview_data=interview_data,    candidate_id="CAND_001",filepath = storage.save_interview_result(# Save}    'test_results': {'passed': 9, 'total': 10}    'final_score': 84.5,    'quality_score': 27.0,    'test_score': 45.0,scoring_data = {}    'conversation_history': [...]    'hints_used': 1,    'communication_score': 90,    'approach_quality': 85,    'stages_completed': ['introduction', 'approach', 'coding', 'review'],    'problem': 'Two Sum',    'interview_id': 'INT_20250129_143022',interview_data = {# Prepare data from interview```python### 2. Save Interview Results```storage = InterviewStorage("custom_path/")# ORstorage = InterviewStorage()  # Default: interview_results/from interview_storage import InterviewStorage```python### 1. Initialize Storage## Python API Usage---```Clamped to [0, 100]Final = test_score + quality_score + approach_score + communication_score - hint_penalty```### Final Score- **Max Hints**: 3 (max penalty = -15 points)- **Formula**: `hints_used * 5` (subtracted)### 5. Hint Penalty  - Response quality  - Question asking  - Clear articulation- **Factors**:- **Formula**: `(communication_score / 100) * 10`### 4. Communication Score (10 points - Chat Mode Only)  - Optimal with edge cases: 95-100/100  - Optimized: 80-95/100  - Brute force: 60-70/100- **Factors**:- **Formula**: `(approach_quality / 100) * 10`### 3. Approach Score (10 points - Chat Mode Only)- **Example**: Quality score 90/100 = 27 points- **Range**: 0-100 from LLaMA 3.3 70B analysis- **Formula**: `(AI_quality_score / 100) * 30`### 2. Code Quality Score (30 points)- **Example**: 9/10 tests passed = 45 points- **Formula**: `(passed_tests / total_tests) * 50`### 1. Test Score (50 points)## Scoring Components---```}  }    }      "interview": "17.5/20"      "quality": "27.0/30",      "tests": "45.0/50",    "breakdown": {    },      ]        "Consider input validation"        "Add edge case handling",      "improvements": [      ],        "Good variable naming"        "Clean code structure",        "Optimal O(n) time complexity",      "strengths": [      "overall_feedback": "Excellent hash map solution...",      "code_quality_score": 90,    "code_quality_details": {    },      "all_passed": false      "total": 10,      "passed": 9,    "test_results": {    "final_score": 84.5,    "hint_penalty": -5,    "communication_score": 9.0,    "approach_score": 8.5,    "quality_score": 27.0,    "test_score": 45.0,  "scoring": {  },    "duration_estimate": 24    "total_messages": 12,    ],      }        "stage": "clarification"        "content": "Can I use a hash map?",        "role": "user",      {      },        "stage": "introduction"        "content": "Let's solve Two Sum...",        "role": "assistant",      {    "conversation_history": [    "hints_used": 1,    "communication_score": 90,    "approach_quality": 85,    ],      "review"      "coding",      "approach",      "clarification",      "introduction",    "stages_completed": [    "language": "python",    "submission_date": "2025-01-29T14:30:22.123456",    "problem": "Two Sum",    "interview_id": "INT_20250129_143022",  "interview_data": {  },    "date": "2025-01-29T14:30:22.123456"    "timestamp": "20250129_143022",    "candidate_id": "CAND_001",  "metadata": {{```json### Complete Interview Record Structure## JSON File Format---- Export all results to single file using admin panel- Each interview is saved with timestamp: `interview_YYYYMMDD_HHMMSS.json`- Each candidate has their own directory (`candidate_id`)```└── all_interviews.json  (export file)│   └── interview_20250129_155530.json│   ├── interview_20250129_141200.json├── CAND_002/│   └── interview_20250129_162450.json│   ├── interview_20250129_150315.json│   ├── interview_20250129_143022.json├── CAND_001/interview_results/```### Directory Layout## Storage Structure---All technical interview results are automatically saved as JSON files for permanent storage and later access. This allows you to review candidate performance, analyze trends, and export data.## OverviewPortals: Candidate, Employee, Admin
...
def show_admin_portal():
    ...
    if page == "🏠 Dashboard":
        ...
        # Import and show results viewer
        from results_viewer_ui import show_interview_results_viewer
